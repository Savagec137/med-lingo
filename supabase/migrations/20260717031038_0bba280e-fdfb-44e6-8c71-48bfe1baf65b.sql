-- === 20260716220000_test_chest_rewards.sql ===
...
-- === 20260716223000_profile_card_customization.sql ===
...
BEGIN
  ...
  IF _item_code NOT IN (
    'game_card_default','game_card_anatomy','game_card_clinical','game_card_surgery',
    'game_card_genome','game_card_neuro','game_card_cardio','game_card_caduceus','game_card_pediatric'
  ) THEN
  ...
  IF _item_code <> 'game_card_default'
    AND NOT EXISTS (
      ...
      WHERE user_id = uid AND item_code = _item_code AND item_type = 'profile_card' AND quantity > 0
    )
    AND NOT EXISTS (
      SELECT 1 FROM public.user_inventory WHERE user_id = uid AND item_code = _item_code
    ) THEN
...
ON CONFLICT (code) DO UPDATE SET
  ...
  rarity = EXCLUDED.rarity, icon = EXCLUDED.icon,
    ...
    || jsonb_build_array(
      jsonb_build_object(
        ...
        'reward', jsonb_build_object(
          'type', 'item', 'amount', 1,
          'item_code', 'game_card_neuro', 'item_type', 'profile_card',
          'rarity', 'legendary',
