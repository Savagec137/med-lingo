...
function hardPrerequisiteFailure(
  ...
  if (blocking) return `Action préalable non réalisée : ${blocking}.`;
  if (action.requires.maxUses !== null) {
    const uses = successfulEntries(session).filter((entry) => entry.actionId === action.id).length;
    if (uses >= action.requires.maxUses) return "Nombre maximal d'utilisations atteint.";
  }
  return undefined;
}
...
export function applyAction(
  ...
  const hardFailure = hardPrerequisiteFailure(session, action);
  if (hardFailure) return refusal(session, actionId, hardFailure);
