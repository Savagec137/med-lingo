...
@media (prefers-reduced-motion: reduce) {
  ...
  .scanline-samu,
  .rain-samu,
  .ecg-pulse-samu { animation: none !important; }
}
.home-reduced-motion .pulse-glow-samu,
.home-reduced-motion .blink-samu,
.home-reduced-motion .float-samu,
.home-reduced-motion .scanline-samu,
.home-reduced-motion .rain-samu,
.home-reduced-motion .ecg-pulse-samu { animation: none !important; }
`;
...
const CITY_BUILDINGS = [55, 70, 42, 90, 60, 80, 48, 100, 65, 85, 45, 75, 52, 88, 40, 95].map(
  (height, index) => ({
    height,
    width: 18 + seededValue(index, 1) * 30,
    opacity: 0.12 + seededValue(index, 2) * 0.15,
  }),
);

const CITY_WINDOWS = Array.from({ length: 28 }, (_, index) => ({
  x: 2 + seededValue(index, 3) * 96,
  y: 30 + seededValue(index, 4) * 120,
  delay: seededValue(index, 5) * 5,
  opacity: 0.15 + seededValue(index, 6) * 0.4,
}));

const RAIN_DROPS = Array.from({ length: 28 }, (_, index) => ({
  left: seededValue(index, 7) * 100,
  delay: seededValue(index, 8) * 4,
  duration: 3 + seededValue(index, 9) * 4,
  height: 15 + seededValue(index, 10) * 30,
}));
  ...
  return (
    <div className="absolute inset-0 pointer-events-none">
      ...
      <div
        ...
        style={{ height: "180px", alignItems: "flex-end" }}
      >
      ...
      {CITY_WINDOWS.map((windowLight, i) => {
        return (
          <div
            ...
            style={{
              left: `${windowLight.x}%`,
              bottom: `${windowLight.y}px`,
              animationDelay: `${windowLight.delay}s`,
              opacity: windowLight.opacity,
            }}
          />
        );
      })}
    ...
    <div className="absolute inset-0 pointer-events-none opacity-70">
      ...
      <div className="absolute top-8 left-1/2 -translate-x-1/2 w-[80%] max-w-[800px] h-[140px] bg-black/60 backdrop-blur-sm border border-cyan-500/10 rounded-xl shadow-[0_0_80px_rgba(6,182,212,0.05)] overflow-hidden">
        ...
        <div className="flex items-center justify-around h-full px-6">
          ...
          <div className="text-center">
            <div className="text-[10px] font-mono text-cyan-400/40 tracking-wider">
              TAUX RÉPONSE
            </div>
