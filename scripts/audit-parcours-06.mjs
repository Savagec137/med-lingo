...
const ALLOWED_PAGES = new Set([23, 24, 25, 26, 27, 71, 72]);
...
const knowledgeFile = await readJson(join(P6_DIR, "knowledge.json"));
...
const legacyDraft = await readJson(join(P6_DIR, "legacy-import-lesson-08.json"));
...
const p6Formation = formation?.parcours?.find((entry) => entry.id === "parcours-06");
const p6Roadmap = roadmap?.parcours?.find((entry) => entry.id === "parcours-06");
const p6Boss = roadmap?.bosses?.find((entry) => entry.id === "dea-p06-boss");
...
if (!p6Formation) addError("MISSING_PARCOURS", "parcours-06", "Absent de formation.json.");
if (!p6Roadmap) addError("MISSING_ROADMAP", "parcours-06", "Absent de roadmap-v2.json.");
if (!p6Boss) addError("MISSING_BOSS", "dea-p06-boss", "Boss absent de roadmap-v2.json.");
...
if (p6Formation?.plannedLessonCount !== 8)
  addError("LESSON_PLAN_MISMATCH", "parcours-06", "8 leçons prévues attendues.");
if (p6Formation?.lessons?.length !== 9)
  addError("MANIFEST_COUNT", "parcours-06", "8 leçons et 1 Boss doivent être enregistrés.");
if (p6Formation?.lessons?.some((entry) => entry.status !== "review")) {
  addError(
...
if (p6Formation?.contentStatus !== "review")
  addError("FORMATION_STATUS", "parcours-06", "contentStatus review attendu.");
if (p6Roadmap?.contentStatus !== "partial")
  addError(
    "ROADMAP_STATUS",
    "parcours-06",
    "Le parcours doit rester partial avant validation formateur.",
  );
if (p6Roadmap?.lessonBlueprints?.some((entry) => entry.status !== "awaiting_content")) {
  addError(
    "BLUEPRINT_STATUS",
    "parcours-06",
...
if (p6Boss?.status !== "awaiting_content")
  addError(
...
if (!legacyDraft?.tags?.includes("import-json-xlsx") || legacyDraft?.items?.length !== 10) {
  addError(
    ...
    "dea-p06-l08",
    "Les 10 questions GPT importées doivent rester archivées.",
  );
}
const manifestEntry = importManifest?.generatedTargets?.find((entry) => entry.id === "dea-p06-l08");
...
for (const knowledge of knowledgeFile?.knowledge || []) {
  ...
  if (!knowledge.plannedParcoursIds?.every((id) => id === "parcours-06"))
    addError(
...
if (knowledgeIds.size !== 12)
  addError(
    "KNOWLEDGE_COUNT",
    "parcours-06",
    "12 connaissances attendues, reçu " + knowledgeIds.size + ".",
  );
...
for (const lesson of lessons.filter(Boolean)) {
  ...
  for (const item of lesson.items || []) {
    ...
    ids.add(item.id);
    const expectedPrefix = lesson.kind === "boss" ? "dea-p06-boss-q" : lesson.id + "-q";
    ...
    if (!item.plannedParcoursIds?.every((id) => id === "parcours-06"))
...
const p6QuestionRefs =
  questionRefs?.questions?.filter((entry) => entry.question_id.startsWith("dea-p06-")) || [];
if (p6QuestionRefs.length !== 72)
  addError("QUESTION_REFERENCE_COUNT", "parcours-06", "72 références globales attendues.");
const p6BossRef = bossRefs?.bosses?.find((entry) => entry.boss_id === "dea-p06-boss");
if (!p6BossRef || p6BossRef.question_ids?.length !== 8)
  addError("BOSS_REFERENCE", "dea-p06-boss", "Référence Boss incomplète.");

addWarning(
  "SOURCE_THRESHOLD_CONFLICT",
  SOURCE_ID,
  "La page 72 utilise deux seuils différents pour l’hypoxémie (95 % puis 93 %). Aucun seuil décisionnel n’a été généré.",
);
addInfo(
  "SOURCE_SELECTION",
  SOURCE_ID,
...
const audit = {
  auditId: "parcours-06-audit-2026-08-11",
  generatedAt: new Date().toISOString(),
  scope: {
    formationId: "dea",
    blocId: "bloc-02",
    parcoursId: "parcours-06",
    title: "Système respiratoire",
  },
  baseline: {
    plannedLessons: 8,
    existingOfficialPlayableQuestions: 0,
    archivedUnverifiedImportedQuestions: 10,
  ...
  result: {
    ...
    lessonQuestionCount: allItems.filter((item) => item.lessonId !== "dea-p06-boss").length,
    bossQuestionCount: allItems.filter((item) => item.lessonId === "dea-p06-boss").length,
    ...
    toolchainDetails: toolchainPassed
      ? {
          ...
          scopedLint: "passed",
          globalLint: "blocked by pre-existing CRLF formatting errors outside the Parcours 6 scope",
        }
  ...
  sources: [
    {
      ...
      pagesUsed: [23, 24, 25, 26, 71, 72],
      excludedPages: [27],
      excludedReason:
        "Vocabulaire redondant ; aucune information supplémentaire nécessaire aux questions.",
    },
  ],
  errors,
  warnings,
  info,
  nextRecommendedParcours: "parcours-07",
};

await writeFile(join(ROOT, "PARCOURS_6_AUDIT.json"), JSON.stringify(audit, null, 2) + "\n", "utf8");
const markdown = [
  "# Rapport de génération — Parcours 6",
  "",
  "## Résultat",
  "",
  "- Parcours : **Système respiratoire** (parcours-06, bloc-02)",
  ...
  "- 10 questions GPT non sourcées conservées sans modification dans une archive historique.",
  ...
  "- Pages 23 à 26 : anatomie, ventilation et échanges gazeux.",
  "- Pages 71 à 72 : fréquence respiratoire et oxymétrie.",
  ...
  "",
  "La page 72 présente deux seuils différents pour l’hypoxémie (95 % puis 93 %). Aucun seuil décisionnel n’a été enseigné. La plage normale 95–100 % est conservée car elle est explicitement donnée, mais doit rester au statut `to_validate`.",
  "",
  "## Contrôles",
  "",
  ...
  "- ESLint global : bloqué par des fins de ligne CRLF préexistantes hors du périmètre du Parcours 6 ; aucun fichier étranger au sprint n’a été reformaté.",
  "",
  "## Boss",
  "",
  ...
  "",
  "Après validation formateur du Parcours 6 : **Parcours 7**.",
  "",
].join("\n");
await writeFile(join(ROOT, "PARCOURS_6_GENERATION_REPORT.md"), markdown, "utf8");
