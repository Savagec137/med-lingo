...
const CHANGE =
  "Ajout contrôlé du Parcours 7 - Système digestif : 13 connaissances, 64 questions de leçon et 8 questions de Boss, toutes à valider.";
...
const p7Knowledge = await readJson(join(P7_DIR, "knowledge.json"));
...
const formationParcours = formation.parcours.find((entry) => entry.id === "parcours-07");
if (!formationParcours) throw new Error("Parcours 7 absent de formation.json.");
Object.assign(formationParcours, {
  ...
  lessons: [
    ...lessonTitles.map((title, index) => ({
      id: "dea-p07-l" + String(index + 1).padStart(2, "0"),
      title,
      kind: "lesson",
      status: "review",
      file: "parcours-07/lesson-" + String(index + 1).padStart(2, "0") + ".json",
    })),
    {
      id: "dea-p07-boss",
      ...
      kind: "boss",
      status: "review",
      file: "parcours-07/lesson-09.json",
    },
  ],
});
...
const roadmapParcours = roadmap.parcours.find((entry) => entry.id === "parcours-07");
if (!roadmapParcours) throw new Error("Parcours 7 absent de roadmap-v2.json.");
...
const normalizedKnowledge = p7Knowledge.knowledge.map((entry) => ({
  ...
  related_topics: [],
  difficulty: "easy",
  tags: ["DEA", "parcours-07", "système-digestif"],
  ...
  [
    {
      ...
      parcours_id: "parcours-07",
      lesson_file: "src/content/formations/dea/parcours-07/lesson-09.json",
...
const digestiveScope = "appareil digestif, pages 39 à 41";
if (!String(catalogDocument.analyzed_scope || "").includes("appareil digestif")) {
  catalogDocument.analyzed_scope = catalogDocument.analyzed_scope
    ? catalogDocument.analyzed_scope + " ; " + digestiveScope
    : digestiveScope;
}
...
const legacyEntry = manifest.generatedTargets.find((entry) => entry.id === "dea-p07-l08");
if (!legacyEntry) throw new Error("Entrée historique dea-p07-l08 absente du manifeste d’import.");
Object.assign(legacyEntry, {
  archiveFile: "parcours-07/legacy-import-lesson-08.json",
  supersededBy: "parcours-07/lesson-08.json",
